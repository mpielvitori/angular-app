...
.directive('ngGallery', ['$document', '$timeout', '$q', '$templateCache', function($document, $timeout, $q, $templateCache) {
	...
	'<div class="{{ baseClass }}">' +
	'  <div ng-repeat="i in images" class="img-thumbnail">' +
	'    <img ng-src="data:image/jpg;base64,{{ i.thumb }}" class="{{ thumbClass }}" ng-click="openGallery($index)" alt="Image {{ $index + 1 }}" />' +
	'  </div>' +
	'</div>' +
	...
	'        <img ng-src="data:image/jpg;base64,{{ i.thumb }}" ng-class="{\'active\': index === $index}" ng-click="changeImage($index)" />' +
	'      </div>' +
	'    </div>' +
	'  </div>' +
	'</div>'
	...
	return {
		...
		link: function (scope, element, attrs) {
			...
			var loadImage = function (i) {
				...
				image.src = 'data:image/jpg;base64,'+scope.images[i].img;
				scope.loading = true;
